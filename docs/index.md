...
=== "CONTENU DE SÉANCE"
    ...
    - Tournoi d'[execubot](https://execubot.fr/gameVersus?instr=on&if=on&elif=on&else=on&for=on&while=on&def=on){target=_blank} ;
...
=== "CONTENU DE SÉANCE"
    ...
    > Si nécessaire, **créer** un compte élève sur [Capytale](https://capytale2.ac-paris.fr/web/c-auth/login){target=_blank} en vous connectant sans ENT avec le code Sésame `ZFK9KPWB3` ;
